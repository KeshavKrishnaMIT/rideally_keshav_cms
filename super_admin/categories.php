...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        if (!$name) {
            ...
            $stmt->execute() ? $success = 'Category added.' : $error = 'Failed. Slug may already exist.';
            $stmt->close();
        }
    }
    ...
    if ($action === 'edit') {
        ...
        if (!$name) {
            ...
            $stmt->execute() ? $success = 'Category updated.' : $error = 'Update failed.';
            $stmt->close();
        }
    }
    ...
    if ($action === 'delete') {
        ...
        $stmt->execute() ? $success = 'Category deleted.' : $error = 'Delete failed.';
        $stmt->close();
    }
}
            ...
            <table class="table table-hover mb-0">
                ...
                <?php $i = 1; while ($c = $cats->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td>
                            <code style="color:var(--primary);font-size:.8rem">
                                <?= sanitize($c['slug']) ?>
                            </code>
                        </td>
                        ...
                        <td>
                            ...
                            <form method="POST" class="d-inline">
                                ...
                                <button
                                    ...
                                    data-confirm="Delete category '<?= sanitize($c['category_name']) ?>'?">
